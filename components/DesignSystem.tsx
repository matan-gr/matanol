...
const getVariantClasses = (variant: Variant, disabled?: boolean) => {
  ...
  switch (variant) {
    case 'primary': return 'bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-600 dark:hover:bg-indigo-500 text-white shadow-md shadow-indigo-500/20 border border-transparent focus:ring-indigo-500';
    case 'secondary': return 'bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-100 border border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 focus:ring-slate-400 shadow-sm';
    case 'danger': return 'bg-white dark:bg-red-950/20 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-900/50 hover:bg-red-50 dark:hover:bg-red-900/30 focus:ring-red-500 shadow-sm';
    ...
    case 'outline': return 'bg-transparent border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 hover:border-slate-400 dark:hover:border-slate-500 hover:bg-slate-50 dark:hover:bg-slate-800 focus:ring-slate-400';
    case 'glass': return 'bg-white/80 dark:bg-slate-900/70 text-slate-800 dark:text-white border border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-900 backdrop-blur-md shadow-sm focus:ring-slate-400';
    default: return 'bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900';
  }
};

const getSizeClasses = (size: Size) => {
  switch (size) {
    case 'xs': return 'px-2 py-1 text-[10px] h-6 rounded';
    case 'sm': return 'px-3 py-1.5 text-xs h-8 rounded-md';
    case 'md': return 'px-4 py-2 text-sm h-10 rounded-lg';
    case 'lg': return 'px-6 py-3 text-base h-12 rounded-xl';
    case 'xl': return 'px-8 py-4 text-lg h-14 rounded-2xl';
    ...
    <div className="w-full">
      <div className="relative group">
        ...
        <input
          ref={ref}
          className={`
            w-full bg-white dark:bg-slate-900 border rounded-lg py-2.5 text-sm text-slate-900 dark:text-slate-100 
            ...
            ${error ? 'border-red-500 focus:border-red-500 focus:ring-red-500/20' : 'border-slate-200 dark:border-slate-700 hover:border-slate-300 dark:hover:border-slate-600'}
            ${className}
          `}
          {...props}
        />
  ...
  return (
    <div className="w-full relative">
      <select
        ref={ref}
        className={`
          w-full appearance-none bg-white dark:bg-slate-900 border rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-slate-100 
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      ...
      <button 
        ...
        className={`
          w-full text-left bg-white dark:bg-slate-900 border rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-slate-100 
...
export const Badge: React.FC<{ children: React.ReactNode, variant?: 'success' | 'warning' | 'error' | 'neutral' | 'info' | 'purple', className?: string }> = ({ children, variant = 'neutral', className = '' }) => {
  ...
  const styles = {
    ...
    error: 'bg-red-50 text-red-700 border-red-200 dark:bg-red-950/40 dark:text-red-400 dark:border-red-800/60',
    ...
    purple: 'bg-violet-50 text-violet-700 border-violet-200 dark:bg-violet-950/40 dark:text-violet-400 dark:border-violet-800/60',
  };

  return (
    <span className={`inline-flex items-center px-2 py-0.5 rounded-md text-[10px] font-bold border tracking-wide shadow-sm ${styles[variant]} ${className}`}>
      {children}
    </span>
  );
};
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <motion.div 
        ...
        className="relative bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-2xl max-h-[85vh] flex flex-col border border-slate-200 dark:border-slate-800"
      >
